...
function unc_divelog_admin_menu() {
    ...
    $main_options_page_hook_suffix = add_menu_page(
        ...
        'unc_divelog_admin_menu', // $menu_slug,
        'unc_divelog_admin_settings' // $function, $icon_url, $position
    );
    add_action('admin_print_scripts-' . $main_options_page_hook_suffix, 'unc_divelog_add_css_and_js');
}
 ...
 */
function unc_divelog_admin_init() {
    global $UNC_DIVELOG;

    add_settings_section(
        'unc_divelog_pluginPage_section',
        __('Settings', 'wordpress'),
        'unc_divelog_settings_section_callback',
        'unc_divelog_settings_page' // need to match menu_slug
    );

    foreach ($UNC_DIVELOG['user_settings'] as $setting => $D) {
        $prefix = $UNC_DIVELOG['settings_prefix'];
        register_setting('unc_divelog_settings_page', $prefix . $setting);
        ...
        if ($D['type'] == 'text') {
            $callback = 'unc_divelog_setting_text_field_render';
        } else {
            $callback = 'unc_divelog_setting_drodown_render';
 ...
 * @param type $A
 */
function unc_divelog_setting_text_field_render($A) {
 ...
 * @param type $A
 */
function unc_divelog_setting_drodown_render($A) {
 ...
 * Called in unc_divelog_admin_init
 */
function unc_divelog_settings_section_callback() {
 ...
 */
function unc_divelog_admin_settings() {
    ...
    settings_fields('unc_divelog_settings_page');
    do_settings_sections( 'unc_divelog_settings_page');
    submit_button();
    echo "</form>\n";
    echo "</div>\n";

    echo "<div id='tab2'>\n";
    echo unc_divelog_admin_upload();
    echo "</div>\n";

    echo "<div id='tab3'>\n";
    echo unc_divelog_admin_display_dives();
    echo "</div>\n";

    echo "<div id='tab4'>\n";
    echo unc_divelog_admin_show_documentation();
    echo "</div>\n";
 ...
 * @global type $UNC_DIVELOG
 */
function unc_divelog_admin_display_dives() {
    global $UNC_DIVELOG;
    $out = "<h2>Manage Dives</h2>\n";
    // get a standard short-tag output for latest date with datepicker
    $out .= unc_display_shortcode(array('options'=> $UNC_DIVELOG['admin_date_selector']));
    echo $out;
}
 ...
 */
function unc_divelog_admin_show_documentation() {
...
function unc_divelog_admin_upload() {
